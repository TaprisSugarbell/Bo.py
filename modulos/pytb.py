...
def pytb_callback_handler(update, context):
    ...
    query.answer()
    query.edit_message_text(text="Enviame link para descargar video")
    return INPUTpy
...
def send_pytb(filename, chat):
    chat.send_action(action=ChatAction.UPLOAD_VIDEO, timeout=None)
    chat.send_video(video=open(filename, "rb"))
    os.unlink(filename)
